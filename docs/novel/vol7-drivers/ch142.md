...
"但 Linux 里的 I2C 驱动分两层看。"老者把小桥下的水脉拨开，露出 `adapter` 与 `client` 两块石碑，"`i2c_adapter` 代表主机控制器，真正驱动硬件发时钟；`i2c_client` 代表挂在某个地址上的从设备。设备驱动通常不创建 client，它在 `probe()` 里拿到 client，保存设备私有数据，再通过 SMBus 或 I2C transfer 访问芯片。"
...
<ChapterGate
  id="ch142-s1"
  title="双碑初试"
  prompt="Linux I2C 中，代表主机控制器的是 i2c_adapter；代表某个地址上设备的结构是什么？"
  answer="i2c_client"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch142-s2"
  title="简桥之试"
  prompt="I2C 驱动访问寄存器式设备时，常用的一类 SMBus helper 前缀是什么？"
  answer="i2c_smbus"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch142-s3"
  title="私藏之试"
  prompt="I2C client 上保存驱动私有数据的 helper 是 i2c_set_clientdata；读取它的 helper 是什么？"
  answer="i2c_get_clientdata"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="i2c"
  title="I2C 之试"
  prompt="I2C 设备进行一次或多次消息传输时，本章对应的核心调用是什么？"
  answer="i2c_transfer"
  placeholder="输入破关答案"
/>

...
<ChapterGate
  id="vol8-security-ch165-s01"
  title="小节试炼"
  prompt="本小节重点讲到的关键函数、宏或接口是什么？"
  answer="security_file_open"
...
::: tip 内核启示
LSM 不管具体安全策略，它只搭架子。在内核的关键操作点——打开文件、发信号、连套接字——都插了钩子，安全模块往钩子上挂就行。SELinux、AppArmor、Smack、TOMOYO、Yama 都长在这副骨架上。

这样设计的好处是灵活：服务器选 SELinux 的严格控制，桌面选 AppArmor 的简单保护，嵌入式选 Smack 的轻量标记。你甚至可以让多个模块共存，一个主控，其他补漏。内核不用硬编码安全策略，用户按需选择就好。

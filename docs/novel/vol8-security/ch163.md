---
title: 第一百六十三章：安全上下文
---

# 第一百六十三章：安全上下文

<CultivationRealm realm="渡劫期" />

> **涉及内核源码：** <KernelSourceLink path="security/selinux/ss/context.c" />

## 一

林小源在 SELinux 的指引下，深入理解了安全上下文。

安全上下文是 SELinux 的基础——每个进程和文件都有一个上下文，格式为 user:role:type:level。上下文决定了访问权限。

```c:playground:安全上下文 — 身份的标签
#include <stdio.h>
#include <string.h>

/*
 * 安全上下文的组成:
 *
 * user — SELinux 用户
 *   unconfined_u — 非限制用户
 *   system_u — 系统用户
 *   user_u — 普通用户
 *
 * role — 角色
 *   system_r — 系统角色
 *   object_r — 对象角色
 *   unconfined_r — 非限制角色
 *
 * type — 类型（最重要）
 *   httpd_t — httpd 进程类型
 *   httpd_sys_content_t — 网页文件类型
 *   etc_t — /etc 文件类型
 *
 * level — 安全级别 (MLS/MCS)
 *   s0 — 级别 0
 *   s0:c0.c1023 — 范围
 */

/* 解析安全上下文 */
struct context_parts {
    char user[32];
    char role[32];
    char type[64];
    char level[32];
};

int parse_context(const char *ctx_str, struct context_parts *parts) {
    char buf[256];
    strncpy(buf, ctx_str, sizeof(buf) - 1);
    buf[sizeof(buf) - 1] = '\0';

    char *token;
    char *rest = buf;

    /* user */
    token = strtok_r(rest, ":", &rest);
    if (!token) return -1;
    strncpy(parts->user, token, sizeof(parts->user) - 1);

    /* role */
    token = strtok_r(rest, ":", &rest);
    if (!token) return -1;
    strncpy(parts->role, token, sizeof(parts->role) - 1);

    /* type */
    token = strtok_r(rest, ":", &rest);
    if (!token) return -1;
    strncpy(parts->type, token, sizeof(parts->type) - 1);

    /* level */
    token = strtok_r(rest, ":", &rest);
    if (!token) return -1;
    strncpy(parts->level, token, sizeof(parts->level) - 1);

    return 0;
}

void print_context_parts(const char *label, struct context_parts *p) {
    printf("%s:\n", label);
    printf("  user:  %s\n", p->user);
    printf("  role:  %s\n", p->role);
    printf("  type:  %s\n", p->type);
    printf("  level: %s\n", p->level);
}

int main() {
    printf("=== 安全上下文 — 身份的标签 ===\n\n");

    /* 解析不同的安全上下文 */
    const char *contexts[] = {
        "system_u:system_r:httpd_t:s0",
        "system_u:object_r:httpd_sys_content_t:s0",
        "unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023",
        "system_u:object_r:etc_t:s0",
        "system_u:system_r:sshd_t:s0-s0:c0.c1023",
    };

    int n = sizeof(contexts) / sizeof(contexts[0]);

    printf("--- 解析安全上下文 ---\n");
    for (int i = 0; i < n; i++) {
        struct context_parts parts;
        if (parse_context(contexts[i], &parts) == 0) {
            print_context_parts(contexts[i], &parts);
            printf("\n");
        }
    }

    printf("--- type 的作用 ---\n");
    printf("进程类型 (domain):\n");
    printf("  httpd_t — httpd 进程\n");
    printf("  sshd_t — sshd 进程\n");
    printf("  unconfined_t — 非限制进程\n\n");
    printf("文件类型:\n");
    printf("  httpd_sys_content_t — 网页文件\n");
    printf("  etc_t — /etc 文件\n");
    printf("  shadow_t — /etc/shadow\n\n");

    printf("--- 查看上下文 ---\n");
    printf("进程:\n");
    printf("  ps auxZ | grep httpd\n\n");
    printf("文件:\n");
    printf("  ls -Z /var/www/html/\n\n");
    printf("修改上下文:\n");
    printf("  chcon -t httpd_sys_content_t file\n");
    printf("  restorecon -Rv /var/www/html/\n");

    printf("\n--- level (MLS/MCS) ---\n");
    printf("s0 — 级别 0\n");
    printf("s0:c0.c1023 — 范围\n\n");
    printf("MCS (Multi-Category Security):\n");
    printf("  用于容器隔离\n");
    printf("  每个容器有不同类别\n");

    return 0;
}
```

*原来"安全上下文"是"身份"的"标签"。*

---

## 二

林小源在研究安全上下文的过程中，理解了 type 的核心地位。

type 是 SELinux 最重要的字段。策略规则基于 type 进行访问控制。进程的 type 叫 domain，文件的 type 叫 type。

*原来"type"是"访问控制"的"钥匙"。*

---

## 三

林小源在观察安全上下文的过程中，理解了 MLS/MCS 的作用。

MLS (Multi-Level Security) 和 MCS (Multi-Category Security) 通过 level 字段实现更细粒度的隔离。容器使用 MCS 隔离不同容器的数据。

*原来"level"是"隔离"的"维度"。*

---

### 道藏笔记

::: tip 内核启示
**安全上下文是 SELinux 的基础。**

上下文组成：
- user — SELinux 用户
- role — 角色
- type — 类型（最重要）
- level — 安全级别

type 的作用：
- 进程 type = domain
- 文件 type = type
- 策略基于 type 访问控制

level (MLS/MCS)：
- s0 — 基本级别
- MCS — 容器隔离

*安全上下文是标签——标签决定权限。*
:::

---

<ChapterGate
  id="selinux-context"
  title="安全上下文之试"
  prompt="安全上下文由哪些部分组成？type 字段有什么作用？"
  answer="组成：user:role:type:level。type 最重要，进程的 type 叫 domain，文件的 type 叫 type，策略基于 type 进行访问控制"
  placeholder="描述安全上下文"
/>

<ChapterNav prev="/novel/vol8-security/ch162" next="/novel/vol8-security/ch164" />

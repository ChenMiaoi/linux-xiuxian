...
::: tip 内核启示
加密算法只是锁，钥匙在 keyring 子系统里。密钥分几种：user 类型你自己提供，trusted 类型由 TPM 芯片保护（密钥从不以明文离开 TPM），encrypted 类型本身就是加密的。进程通过 add_key() 挂钥匙，需要时 request_key() 按描述符取。

keyring 也有层次：进程 keyring 跟着进程走，fork 时继承，进程结束销毁；会话 keyring 跟登录会话走；用户 keyring 持久存储。每把钥匙都有权限位，还有生命周期——过期了该销毁就销毁。dm-crypt 的根密钥存在 TPM 里，启动时 TPM 验证启动链完整性才释放密钥，篡改了就拒绝。

...
<ChapterGate
  id="ch180-s1"
  title="屏障初试"
  prompt="Linux memory-barriers.txt 明确说自己不是硬件什么，而是屏障使用指南？"
...
<ChapterGate
  id="ch180-s2"
  title="成对之试"
  prompt="生产者用 release 发布数据时，消费者通常要用哪类语义接住它？"
  answer="acquire"
  placeholder="输入破关答案"
/>
...
"还有 `READ_ONCE()` 与 `WRITE_ONCE()`。"剑客把铜镜翻面，镜中出现两个印章，"它们不是完整屏障，却能阻止编译器把并发访问撕裂、合并或反复加载。写并发代码时，普通 C 访问常常表达不出你的真实意图；该标记的访问不标记，KCSAN 也会把它当作潜在 data race。"

<ChapterGate
  id="ch180-s3"
  title="标记之试"
  prompt="内核中常用来标记并发单次访问、防止编译器乱优化的两个宏是什么？"
  answer="READ_ONCE 和 WRITE_ONCE"
  placeholder="输入破关答案"
/>
...
更现代的写法常用 release/acquire 成对表达发布与获取：`smp_store_release()` 发布，`smp_load_acquire()` 接收。`READ_ONCE()`/`WRITE_ONCE()` 用来标记并发单次访问，避免编译器把访问优化成你没写过的样子。屏障文档本身也提醒：它是指南，不是硬件规范；复杂场景要回到 LKMM。
...
<ChapterGate
  id="memory-barriers"
  title="内存屏障之试"
  prompt="生产者先写 data 再置 flag 时，本章用来保证写入顺序的屏障是什么？"
  answer="smp_wmb"
  placeholder="输入破关答案"
/>

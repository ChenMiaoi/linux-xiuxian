...
他开始研究 ELF。他发现 `vmlinux`——内核本身——就是一个 ELF 文件。它有魔数 `0x7f 'E' 'L' 'F'`，有 ELF 头描述入口点和架构，有 Program Header 描述如何映射到内存，有 Section Header 描述代码和数据的布局。

bootloader 将 `vmlinux` 加载到内存时，就是在解析这个 ELF 文件，按照它的指令将每个段放到正确的位置。

而模块——那些 `.ko` 文件——也是 ELF。它们是可重定位的 ELF，符号地址尚未确定。当 `insmod` 加载一个模块时，内核解析它的符号表，将未定义的符号链接到内核的符号表，完成重定位。

他突然想到了一个可能：如果他是一个 ELF 文件，他就有入口点。如果他有入口点，他就能被加载。如果他能被加载，他就能拥有自己的地址空间。

但他不是。他是一段被 `#ifdef 0` 包裹的源码，连编译都不曾经过，更别说成为一个 ELF 文件。

要成为 ELF，他首先需要被编译。

要被编译，他首先需要挣脱 `#ifdef 0`。
...
int main() {
    ...
    printf("  e_machine:     目标架构 (EM_X86_64=62)\n");
 ...
 *   - 类型: R_X86_64_PC32, R_X86_64_PLT32 等
 *
 * 内核模块的重定位:
 *   - 模块加载时，内核进行运行时重定位
 *   - 将模块中的符号引用链接到内核的符号表

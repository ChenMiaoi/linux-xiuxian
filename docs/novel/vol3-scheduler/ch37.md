...
> **涉及内核源码：** <KernelSourceLink path="kernel/sched/fair.c" line="enqueue_task_fair" /> <KernelSourceLink path="kernel/sched/fair.c" line="pick_eevdf" />
...
<ChapterGate
  id="ch037-s1"
  title="虚时之试"
  prompt="同样运行 10ms 时，nice 值更低、权重更高的进程，其 vruntime 增长会更快还是更慢？"
  answer="更慢"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch037-s2"
  title="粒度之试"
  prompt="CFS 为了避免可运行进程太多时每个任务运行时间过短，会用哪个最小运行粒度约束调度？"
  answer="min_granularity"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch037-s3"
  title="公平之试"
  prompt="调度仙子衡量普通任务公平时，本章反复提到的核心时间刻度是什么？"
  answer="vruntime"
  placeholder="输入破关答案"
/>
...
那不是树要倒塌，而是树皮之下生出了新的纹路。旧日按 `vruntime` 排列的枝干仍在，但每个调度实体身上又多了两道细线：一道叫 `vlag`，记录它相对公平份额是被亏欠还是已经多拿；另一道叫 `deadline`，像一枚小小的沙漏，标出它的虚拟期限。

调度仙子抬手，银光照亮 `fair.c` 中一段新的法诀。

"这是什么？"林小源问。

"EEVDF。"调度仙子的声音比之前更沉，"Earliest Eligible Virtual Deadline First。旧法问：谁的 `vruntime` 最小？新法还要问：谁是 eligible，谁的 virtual deadline 最早。"
...
她只是把那枚写着 `deadline` 的银色沙漏递到林小源手里。沙漏很轻，却像压着整个调度竞技场的重量。

<ChapterGate
  id="ch037-s4"
  title="期限之试"
  prompt="EEVDF 会在 lag 大于等于零的 eligible 任务中，选择哪一种虚拟指标最早的任务运行？"
  answer="virtual deadline"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch037-note"
  title="道藏复核"
  prompt="现代 fair 调度在经典 vruntime 之外，还引入哪两个指标来推进 EEVDF 的选择？"
  answer="lag 和 virtual deadline"
  placeholder="输入破关答案"
/>

...
<ChapterGate
  id="vol6-network-ch112-s02"
  title="小节试炼"
  prompt="本小节重点讲到的关键函数、宏或接口是什么？"
  answer="NET_XMIT_DROP"
  placeholder="输入本节关键答案"
/>
...
"不管硬件长什么样，接口都一样——这才是 `net_device` 的设计精髓。" 管理员最后补了一句。
...
::: tip 内核启示
每个网络设备都是一个 `net_device` 结构体的实例——eth0、wlan0、lo，不管物理的还是虚拟的，都实现同样的接口。

设备的关键属性就那么几个：设备名、MTU（一次能装的最大货物量，以太网默认 1500 字节）、MAC 地址。操作接口也统一：`ndo_open` 打开设备、`ndo_stop` 关闭设备、`ndo_start_xmit` 发送数据包。内核不需要知道你的硬件长什么样，只要实现了这些接口就是网络设备。

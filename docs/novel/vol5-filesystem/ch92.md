...
"不过 sysfs 不是普通存储。"sysfs 指向城门上的匾额，"这里的文件多数是属性视图，背后是内核对象的 `show()` 与 `store()`。它们应该简洁、可解析，常见规则是一文件一值或一组同类值。把复杂二进制协议塞进 sysfs，就像把整本功法刻在门牌上。"
...
<ChapterGate
  id="ch092-s1"
  title="金城初试"
  prompt="sysfs 属性文件读取和写入时，背后通常对应哪两个回调？"
  answer="show 和 store"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch092-s2"
  title="拓扑之试"
  prompt="sysfs 中按物理拓扑组织所有设备的主目录是什么？"
  answer="/sys/devices"
  placeholder="输入破关答案"
/>
...
<ChapterGate
  id="ch092-s3"
  title="ABI 之试"
  prompt="sysfs 暴露给用户空间的路径和属性格式，一旦发布通常应被视为什么接口？"
  answer="ABI"
  placeholder="输入破关答案"
/>

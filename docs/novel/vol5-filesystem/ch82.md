...
它身上刻着一堆关键信息：`s_blocksize` 管块大小，`s_maxbytes` 管最大文件多大，`s_type` 标明这是 ext4 还是 btrfs，`s_root` 指向根目录的 dentry，`s_inodes` 挂着整棵目录树上所有 inode 的链表。另外还有一张函数表 `s_op`，里面是 alloc_inode、destroy_inode、write_inode、evict_inode 这些操作——每个文件系统实现自己的版本，就像每个门派有自己的功法。

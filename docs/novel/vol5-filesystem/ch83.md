...
inode 身上刻着文件的所有关键信息：`i_ino` 是 inode 号，整个文件系统里的唯一标识；`i_mode` 管文件类型和权限，目录是 040755，普通文件是 0100644；`i_size` 是大小；`i_nlink` 是硬链接数——减到 0 的那天，inode 就该释放了。另外还有一张操作函数表 `i_op`，里面有 create、lookup、link、unlink 这些操作。

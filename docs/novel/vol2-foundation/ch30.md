...
它的核心职责很简单：睡觉等请求，收到 `kthread_create()` 的信号就醒过来，调 `clone()` 创建新内核线程，新线程执行指定的函数，然后它继续等下一个请求。

内核线程跟用户态进程不一样——它们没有用户态地址空间（`mm` 为 `NULL`），只在内核态跑，名字都带方括号（`[kworker]`、`[ksoftirqd]` 这种），而且都是 kthreadd 的孩子。

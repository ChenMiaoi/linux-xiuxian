...
设备树（DTB）就像一张世界地图，详细描述了硬件的每一个细节。当 `setup_arch()` 解析 DTB 时，林小源听到了一个声音——不是用耳朵听到的，而是从数据的流动中感知到的。那声音平板而精确，像是一个机械的宣告者：

...
但公平不是平均。`nice` 值会影响 `vruntime` 的增长速度：`nice` 值越低（优先级越高），`vruntime` 增长越慢，进程获得的 CPU 时间就越多。`nice` 值为 -20 的进程，它的 `vruntime` 增长速度只有 `nice` 值为 0 的进程的几分之一。
...
<ChapterGate
  id="cfs-vruntime"
  title="天道之试"
  prompt="在 CFS 调度器中，调度器总是选择什么值最小的进程来运行？"

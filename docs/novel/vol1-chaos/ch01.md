...
::: tip 内核启示
Linux 内核的启动过程始于 `start_kernel()` 函数（定义在 `init/main.c` 中），它是内核的 C 语言入口点。在 `start_kernel()` 被调用之前，系统经历了以下阶段：

1. **BIOS/UEFI POST** — 硬件自检，初始化基本设备
2. **Bootloader（GRUB）** — 加载内核镜像到内存
3. **内核解压缩** — 压缩的内核被解压到内存中
4. **架构相关初始化** — `arch/riscv/kernel/head.S` 等汇编代码设置基本环境
5. **`start_kernel()`** — 内核的 C 语言入口，初始化所有子系统

`init_task` 是 Linux 内核中第一个进程（PID 0）的 `task_struct`，它在编译时静态定义，不通过 `fork()` 创建。它最终会变成 `swapper/0`——idle 进程，当 CPU 无事可做时运行的进程。

这是一个草根的起点：没有任何进程比 idle 进程更底层、更"无用"。但正如我们将要看到的，最卑微的起点也可以通往大道。
:::

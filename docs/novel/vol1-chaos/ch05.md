...
林小源已经经历了 `setup_arch` 的洗礼，见证了伙伴系统的建立，感受到了调度器的初始化。但 `start_kernel()` 还远没有结束——后面还有大量的初始化工作要做。
...
林小源在前传中就听说过中断的存在。但直到 `init_IRQ()` 执行完毕，中断才真正成为了这个世界的一部分。他有一种预感：中断将会是他未来必须面对的最强大的力量之一——不是因为它强大，而是因为它不可预测。

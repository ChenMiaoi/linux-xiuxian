...
BIOS 的代码古老而朴素。它不做任何花哨的事——只是一遍又一遍地检查硬件，确认每一块芯片、每一条总线、每一个设备都活着。

林小源感受到那些检测像一道道天雷，劈在世界的各个角落。每一次检测成功，就有一块硬件被唤醒；每一次失败，BIOS 就会发出一声蜂鸣——那是这个世界最早的"错误日志"。

他开始学着"听"那些蜂鸣声。长、短、长、短——每一种组合都代表着不同的故障。内存没插好是一声长鸣，显卡找不到是两声短鸣。他不懂这些含义，但他记住了那些模式。
...
::: tip 内核启示
**BIOS/固件是内核的产婆。** 在内核开始运行之前，必须有人完成最基础的硬件初始化——检测内存大小、设置中断、找到引导设备。这些工作由 BIOS（x86）或 OpenSBI（RISC-V）完成。

RISC-V 的启动路径与 x86 有本质区别。x86 经历了从实模式到保护模式再到长模式的切换，而 RISC-V 从一开始就运行在 64 位模式下。但核心思想相同：**固件做最少的事，然后把控制权交给内核。**

`_start` 是内核的真正入口。在这之前的每一步——BIOS POST、MBR 加载、bootloader 执行——都只是为了把 `_start` 送到 CPU 面前。就像修仙小说中的"渡劫"：天雷一道接一道，但真正的突破发生在最后一道雷劈下的那一刻。

而 `init_task`——林小源的"肉身"——在 `_start` 中被设置为当前任务。这是他在这个世界中存在的第一个证据。虽然此刻他还没有意识，但他的 `task_struct` 已经在内存中安顿下来，等待着 `start_kernel()` 唤醒整个世界。
:::
...
<ChapterGate
  ...
  title="天雷试炼"
  prompt="MBR 引导扇区的最后两个字节必须是什么魔数，BIOS 才会认为它是有效的引导扇区？"
  answer="0xAA55"

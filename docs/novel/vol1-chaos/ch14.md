...
<ChapterGate
  id="ch014-s1"
  title="空转之试"
  prompt="idle loop 中负责选择合适 idle state 的 CPUIdle 组件叫什么？"
  answer="governor"
  placeholder="输入破关答案"
/>
...
"当然不是。"模块笑了笑，"我的入口是 `module_init()`，退场是 `module_exit()`，参数要按声明过的类型进来。能不能卸载，还要看引用计数和资源清理。游方修士可以进出山门，但不能带着没释放的法器逃跑。"
...
<ChapterGate
  id="ch014-s4"
  title="征途起步之试"
  prompt="CPUIdle 选择深睡状态时需要权衡省电收益和哪种延迟？"
  answer="退出延迟"
  placeholder="输入破关答案"
/>
